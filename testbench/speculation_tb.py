...
def run_eval(
    ...
    secondary_assistant_model: Optional["PreTrainedModel"] = None
):
...
def get_model_answers(
    ...
    if use_pp:
        ...
        if assistant_model!=None:
            assistant_model_1, tokenizer = load_model(
            assistant_model,
            use_flash=use_flash,
            device=f"cuda",
            device_map="balanced",
        ...
        if secondary_assistant_model!=None:
            assistant_model_2, tokenizer = load_model(
        ...
        if assistant_model!=None:
            ...
            assistant_model_1 = deepspeed.init_inference(
                assistant_model_1,
                mp_size=int(os.getenv("WORLD_SIZE", "1")),
                dtype=torch.half
            )
        if secondary_assistant_model!=None:
            ...
            assistant_model_2 = deepspeed.init_inference(
                assistant_model_2,
                mp_size=int(os.getenv("WORLD_SIZE", "1")),
                dtype=torch.half
            )
        ...
        if assistant_model!=None:
            assistant_model_1.tokenizer = tokenizer
        if secondary_assistant_model!=None:
            assistant_model_2.tokenizer = tokenizer
    ...
    for question_idx, question in enumerate(tqdm(questions)):
        ...
        for i in range(num_choices):
            ...
            for j in range(len(question["turns"])):
                ...
                if True:
                    ...
                    if assistant_model!=None and secondary_assistant_model!=None:
                        output_ids = model.generate(
                            torch.as_tensor(input_ids).cuda(), assistant_model=assistant_model_1, secondary_assistant_model=assistant_model_2,
                            do_sample=do_sample,
                    ...
                    elif assistant_model!=None and secondary_assistant_model==None:
                        output_ids = model.generate(
                            torch.as_tensor(input_ids).cuda(), assistant_model=assistant_model_1,
                            do_sample=do_sample,
                            temperature=temperature,
                            max_new_tokens=max_new_token,
                            top_k=0.0, top_p=1.0, pad_token_id=tokenizer.eos_token_id
        ...
        if True or (lade.get_device() == 0 and ds_local_rank == 0):
            # Dump answers
    ...
    if False and (lade.get_device() == 0 and ds_local_rank == 0):
...
if __name__ == "__main__":
    ...
    file_handler = py_logging.FileHandler(f"{args.log_dir}/{args.model_id}.log")
    console_handler = py_logging.StreamHandler()
    console_handler.setLevel(py_logging.WARNING)
    
    logging.disable_default_handler()
    logging.add_handler(file_handler)
    logging.add_handler(console_handler)
    logger.info(f"Start running {args.model_id}")
    ...
    run_eval(
        ...
        secondary_assistant_model=args.secondary_assistant_model,
    )
    # ds_local_rank = int(os.getenv('LOCAL_RANK', '0'))
    # if lade.get_device() == 0 and ds_local_rank == 0:
