...
def generate_test_script(list_tuples):
    ...
    for model, assistant_model, digest in list_tuples:
        script = f"python -m speculation_tb --model-path {model} --assistant-model-path {assistant_model} --model-id \"{digest}\" --bench-name spec_bench --temperature 0.0 --dtype float16 --log-dir log --question-begin 479"
